...
// infinityCicle();
/*Потому-что дробные вычисления в бинарном формате имеют остаток, который накапливается и приводит неожиданным ерзультатам. В этом случае результат просто "перепрыгивает" через ожидаемое значение. Этот эффект называется - "потеря точности". Дабы этого избежать стоит на каждом цикле округлять значение до одного знака после запятой*/
